...
const WebCryptoModule = require("./webCryptoToCrypto");
let webCrypto = new WebCryptoModule();
try {
  if (crypto && crypto.subtle) {
    webCrypto = crypto.subtle;
  }
} catch (e) {
  console.log("Web Crypto not found");
}
...
async function digestMessage(algorithm, message) {
  const digest = await webCrypto.digest(
    algorithm,
...
function SHA1() {
  this.getHash = async function(xml) {
    const digest = await digestMessage({ name: "SHA-1" }, xml);
...
function SHA256() {
  this.getHash = async function(xml) {
    const digest = await digestMessage({ name: "SHA-256" }, xml);
    return digest;
  };
...
function SHA512() {
  this.getHash = async function(xml) {
    return await digestMessage({ name: "SHA-512" }, xml);
  };
...
async function signMessage(algorithm, key, message) {
  //const privateJwk = keyTo.from(key.toString(), "pem").toJwk("private");
  let keyToUse;
  if (typeof cryptoModule !== "undefined") {
    keyToUse = key;
  } else {
    keyToUse = await webCrypto.importKey(
      "pkcs8",
      convertPemToBinary(key.toString()),
      algorithm,
      true,
      ["sign"]
    );
  }
  const digest = await webCrypto.sign(
    algorithm,
    keyToUse,
    new TextEncoder().encode(message)
  );
...
function base64StringToArrayBuffer(b64str) {
  ...
  if (typeof atob !== "undefined") {
    ...
    byteStr = Buffer.from(b64str, "base64").toString();
  }
...
async function verifySignatureForValue(algorithm, key, signature, value) {
  //console.log("VERIFYING IN ", key, signature, value);
  let keyToUse;
  if (typeof cryptoModule !== "undefined") {
    keyToUse = key;
  } else {
    ...
    const publicKey = await keyStore.add(certificateString, "pem");

    const pem = publicKey.toPEM(false).replace(/\r\n/g, "\n");
    keyToUse = await webCrypto.importKey(
      "spki",
...
function RSASHA1() {
   ...
   *
   */
  this.getSignature = async function(signedInfo, signingKey) {
   ...
   *
   */
  this.verifySignature = async function(str, key, signatureValue) {
    return await verifySignatureForValue(
      { name: "RSASSA-PKCS1-v1_5", hash: { name: "SHA-1" } },
      key,
      signatureValue,
      str
  ...
  this.verifySignature = async function(str, key, signatureValue) {
    ...
    if (typeof cryptoModule !== "undefined") {
      ...
      if (typeof key === "string") {
        key = new TextEncoder().encode(btoa(key));
      }
      keyToUse = await webCrypto.importKey("raw", key, true, [
        "sign",
        "verify"
      ]);
    }
    ...
    return await webCrypto.verify(
      ...
      keyToUse,
      new TextEncoder().encode(signatureValue),
  ...
  this.getSignature = async function(signedInfo, signingKey) {
    ...
    if (typeof cryptoModule !== "undefined") {
      ...
      if (typeof signingKey === "string") {
        signingKey = new TextEncoder().encode(btoa(signingKey));
      }
      keyToUse = await webCrypto.importKey("raw", signingKey, true, [
        "sign",
        "verify"
      ]);
    }
...
SignedXml.prototype.checkSignature = async function(xml) {
  this.validationErrors = [];
  this.signedXml = xml;
...
SignedXml.prototype.validateSignatureValue = async function(doc) {
  ...
  var res = await signer.verifySignature(
    signedInfoCanon,
    this.signingKey,
    this.signatureValue
  );
  ...
  for (var r in this.references) {
    ...
    if (!validateDigestValue(digest, ref.digestValue)) {
      if (ref.inclusiveNamespacesPrefixList) {
        ...
        digest = await hash.getHash(canonXml);
 ...
 *
 */
SignedXml.prototype.computeSignature = async function(xml, opts) {
  var doc = new Dom().parseFromString(xml),
    xmlNsAttr = "xmlns",
    signatureAttrs = [],
    location,
  ...
  var signedInfo = await this.createSignedInfo(doc, prefix);
  this.signatureXml += signedInfo;
  this.signatureXml += await this.createSignature(signedInfo, prefix);
 ...
 *
 */
SignedXml.prototype.createReferences = async function(doc, prefix) {
  var res = "";
  ...
  for (var n in this.references) {
    ...
    for (var h in nodes) {
      ...
      const digest = await digestAlgorithm.getHash(canonXml);
      res +=
        "</" +
        prefix +
        "Transforms>" +
 ...
 *
 */
SignedXml.prototype.createSignedInfo = async function(doc, prefix) {
 ...
 *
 */
SignedXml.prototype.createSignature = async function(signedInfo, prefix) {
  var xmlNsAttr = "xmlns";
  ...
  this.signatureValue = await signatureAlgorithm.getSignature(
    canonizedSignedInfo,
    this.signingKey
  );
  return (
