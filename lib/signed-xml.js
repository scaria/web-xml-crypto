...
async function verifyRSASignature(key, md, signature) {
  let keyString;
  //Signing key is a buffer of private key pem or Uint8Array
  if (key instanceof Uint8Array) {
    keyString = new TextDecoder().decode(key);
...
function RSASHA1() {
  ...
  this.getSignature = async function(signedInfo, signingKey) {
    ...
    return await signRSAMessage(md, signingKey);
  };
  ...
  this.verifySignature = async function(str, key, signatureValue) {
    ...
    return await verifyRSASignature(key, md, signatureValue);
  };
...
function RSASHA256() {
   ...
   *
   */
  this.verifySignature = async function(str, key, signatureValue, log) {
    const md = forge.md.sha256.create();
    md.update(str, "utf8");
...
SignedXml.prototype.checkSignature = async function(xml, log = () => {}) {
  this.validationErrors = [];
  this.signedXml = xml;
  this.log = log;
  lllog = log;
  ...
  if (!(await this.validateSignatureValue(doc, log))) {
    return false;
  }
...
SignedXml.prototype.validateSignatureValue = async function(
  doc,
  log = () => {}
) {
