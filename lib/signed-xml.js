var xpath = require("xpath"),
  ...
  webCrypto = require("@trust/webcrypto").subtle,
...
function SHA256() {
  this.getHash = async function(xml) {
    return await digestMessage({ name: "SHA-256" }, xml);
  };
...
async function signMessage(algorithm, key, message) {
  //console.log(`Signing `, algorithm);
  const privateJwk = keyTo.from(key.toString(), "pem").toJwk("private");

  const importedKey = await webCrypto.importKey(
    "jwk",
    privateJwk,
    Object.assign({ saltLength: 0 }, algorithm),
    false,
    ["sign"]
  );

  //console.log(`With Key `, importedKey);
  //console.log("Algorithm : ", JSON.stringify(importedKey.algorithm));
...
async function verifySignature(algorithm, key, signature, value) {
  ...
  const publicJwk = keyTo.from(pem, "pem").toJwk("public");
  console.log(pem);
  const importedKey = await webCrypto.importKey(
    "jwk",
    publicJwk,
...
function HMACSHA1() {
  this.verifySignature = async function(str, key, signatureValue) {
    ...
    const importedKey = await webCrypto.importKey("raw", key, true, [
      "sign",
      "verify"
    ]);
  ...
  this.getSignature = async function(signedInfo, signingKey) {
    ...
    const importedKey = await webCrypto.importKey("raw", signingKey, true, [
      "sign",
      "verify"
    ]);
    const digest = await webCrypto.sign(
  ...
  for (var n in this.references) {
    ...
    for (var h in nodes) {
      ...
      res +=
        ...
        prefix +
        "DigestValue>" +
        (await digestAlgorithm.getHash(canonXml)) +
        "</" +
        prefix +
        "DigestValue>" +
        "</" +
